...
/*
[J0]
position_limit_max=90
position_limit_min=90
velocity_limit=50
torque_limit=50
max_motor_speed = 2222
max_motor_torque = 3333
[J1]
position_limit_max=90
position_limit_min=90
velocity_limit=50
...
typedef enum {
    ...
    J9,
    UNKNOWN_SECTION
} limits_ini_config_section_t;

static const char *limits_ini_config_sections[UNKNOWN_SECTION + 1] = {
    "J0",
    "J1",
    "J2",
    "J3",
...
static limits_ini_config_section_t get_section(const char *section) {
    if (MATCH(limits_ini_config_sections[J0], section)) {
        return J0;
    } else if (MATCH(limits_ini_config_sections[J1], section)) {
        return J1;
    } else if (MATCH(limits_ini_config_sections[J2], section)) {
        return J2;
    } else if (MATCH(limits_ini_config_sections[J3], section)) {
        return J3;
    } else if (MATCH(limits_ini_config_sections[J4], section)) {
        return J4;
    } else if (MATCH(limits_ini_config_sections[J5], section)) {
        return J5;
    } else if (MATCH(limits_ini_config_sections[J6], section)) {
        return J6;
    } else if (MATCH(limits_ini_config_sections[J7], section)) {
        return J7;
    } else if (MATCH(limits_ini_config_sections[J8], section)) {
        return J8;
    } else if (MATCH(limits_ini_config_sections[J9], section)) {
        return J9;
    } else {
...
bool check_limits_ini_exists(void) {
    FILE *fp;
    fp = fopen(LIMITS_INI_FILENAME, "r");
    if (fp == NULL) {
        return false;
    }
    fclose(fp);
...
gberror_t config_parser(const char *section, const char *key,
                        const char *value, void *config_v) {
    limits_ini_config_section_t s = get_section(section);
...
uint8_t get_limits_ini_sections(void) {
    ...
    char section[40];
    uint8_t count = 0;
    for (s = 0; ini_getsection(s, section, sizeof section, LIMITS_INI_FILENAME) > 0; s++) {
        count++;
    }
    return count;
}
...
gberror_t read_limits_ini(uint8_t *number_of_items_found) {
    if (!ini_browse(config_parser, &map_machine_limits, LIMITS_INI_FILENAME)) {
