...
void adhoc_msg_processing(void) {
    ...
    request_id = output_struct->m_request.request_id;
...
//    memcpy(input_struct->ecm_response.request_id, request_id, DPM_REQUEST_ID_LENGTH);
    strncpy(input_struct->m_response.request_id, request_id, DPM_REQUEST_ID_LENGTH);
    ...
    memset(input_struct->m_response.data, 0, DPM_REQUEST_RESPONSE_DATA_LENGTH);
    rc = json_msg_request_respond_handle((char *) output_struct->m_request.data, input_struct->m_response.data,
                                         request_id);
