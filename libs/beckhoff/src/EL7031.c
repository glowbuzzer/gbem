...
gberror_t ec_pdo_map_el7031(const uint16_t slave) {
    ...
    if (ec_printSDO) {
        UM_INFO(GBEM_UM_EN, "GBEM: SODs configured for PDO mapping for EL7031 slave [%u] are:",
                slave);
    } else {
...
uint16_t ec_get_stat_wrd_el7031(const uint16_t drive) {
    ...
    el7031_drive_state_bits[drive] = ec_decode_drive_state_el7031(
            ec_pdo_get_input_uint16(map_drive_to_slave[drive], EL7031_STATUSWORD_PDO_INDEX));
    ...
    if (el7031_drive_state[drive] == CIA_SWITCHED_ON || el7031_drive_state[drive] == CIA_READY_TO_SWITCH_ON ||
        el7031_drive_state[drive] == CIA_OPERATION_ENABLED) {
