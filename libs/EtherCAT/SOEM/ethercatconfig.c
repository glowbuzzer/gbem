...
//   LL_INFO(GBEM_GEN_LOG_EN, "GBEM: wkc in ecx_detect_slaves after BWRs %d",wkc);
//   printf("wkc in ecx_detect_slaves after BWRs %d\n",wkc);
    if (wkc > 0) {
        /* this is strictly "less than" since the master is "slave 0" */
        if (wkc < EC_MAXSLAVE) {
            *(context->slavecount) = wkc;
        } else {
            UM_INFO(GBEM_UM_EN, "GBEM: Error: too many slaves on network: num_slaves=%d, EC_MAXSLAVE=%d\n",
                    wkc, EC_MAXSLAVE);
            return -2;
        }
    }
...
static void ecx_set_slaves_to_default(ecx_contextt *context) {
    ...
    b = 0x00;
    ecx_BWR(context->port, 0x0000, ECT_REG_DCSYNCACT, sizeof(b), &b,
            EC_TIMEOUTRET3);     /* reset activation register */
...
//    LL_INFO(GBEM_GEN_LOG_EN,"GBEM: wkc after ecx_detect_slaves (less than zero is bad) %d",wkc);
//            printf("wkc after ecx_detect_slaves (less than zero is bad) %d",wkc);
    if (wkc > 0) {
        ...
        for (slave = 1; slave <= *(context->slavecount); slave++) {
            ...
            ecx_APWRw(context->port, ADPh, ECT_REG_STADR, htoes(slave + EC_NODEOFFSET),
        ...
        for (slave = 1; slave <= *(context->slavecount); slave++) {
            ...
            topology = ecx_FPRDw(context->port, configadr, ECT_REG_DLSTAT,
            ...
            ecx_FPWRw(context->port, configadr, ECT_REG_ALCTL, htoes(EC_STATE_PRE_OP | EC_STATE_ACK),
...
static int ecx_map_coe_soe(ecx_contextt *context, uint16 slave, int thread_n) {
    ...
    EC_PRINT(" >Slave %d	 %x, state %2.2x, name %s\n",
    ...
    if ((*(context->slavecount) > 0) && (group < context->maxgroup)) {
        ...
        for (slave = 1; slave <= *(context->slavecount); slave++) {
            ...
            if (!group || (group == context->slavelist[slave].group)) {
                ...
                ecx_FPWRw(context->port, configadr, ECT_REG_ALCTL, htoes(EC_STATE_SAFE_OP),
 ...
 * @param[in] usetable    = TRUE when using configtable to init slaves, FALSE otherwise
 * @param[out] pIOmap     = pointer to IOmap
 * @param[in] IOmap_size  = max size of IOmap
 * @return Workcounter of slave discover datagram = number of slaves found
 * @warning GB added IOmap_size December 2020
 */
int ec_config(uint8 usetable, void *pIOmap, int IOmap_size) {
    int wkc, usedmem;
    wkc = ec_config_init(usetable);
    if (wkc) {
        usedmem = ec_config_map(pIOmap);
        if (usedmem >= IOmap_size) {
            UM_FATAL("Size of slave io map is greater than the maximum we allow");
        }
