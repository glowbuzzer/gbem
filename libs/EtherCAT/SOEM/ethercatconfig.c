...
//   printf("wkc in ecx_detect_slaves after BWRs %d\n",wkc);
   if (wkc > 0)
   {
      ...
      {
         UM_INFO(GBEM_UM_EN, "Error: too many slaves on network: num_slaves=%d, EC_MAXSLAVE=%d\n",
               wkc, EC_MAXSLAVE);
         return -2;
      }
   }
...
static int ecx_map_coe_soe(ecx_contextt *context, uint16 slave, int thread_n)
{
   ...
   EC_PRINT(" >Slave %d	 %x, state %2.2x, name %s\n",
            slave, context->slavelist[slave].configadr, context->slavelist[slave].state, context->slavelist[slave].name);
