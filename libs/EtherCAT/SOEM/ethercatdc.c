...
void ecx_dcsync0(ecx_contextt *context, uint16 slave, boolean act, uint32 CyclTime, int32 CyclShift) {
    ...
    rc = ecx_FPWR(context->port, slaveh, ECT_REG_DCSYNCACT, sizeof(RA), &RA, EC_TIMEOUTRET);
    if (rc <= 0) {
        printf("Error writing: ECT_REG_DCSYNCACT\n");
    }
    ...
    h = 0;
    rc = ecx_FPWR(context->port, slaveh, ECT_REG_DCCUC, sizeof(h), &h, EC_TIMEOUTRET); /* write access to ethercat */
    if (rc <= 0) {
        printf("Error writing: ECT_REG_DCCUC\n");
    }


    t1 = 0;
    rc = ecx_FPRD(context->port, slaveh, ECT_REG_DCSYSTIME, sizeof(t1), &t1,
                  EC_TIMEOUTRET); /* read local time of slave */
    if (rc <= 0) {
        printf("Error writing: ECT_REG_DCSYSTIME\n");
    }
    t1 = etohll(t1);
    ...
    rc = ecx_FPWR(context->port, slaveh, ECT_REG_DCSTART0, sizeof(t), &t, EC_TIMEOUTRET); /* SYNC0 start time */
    if (rc <= 0) {
        printf("Error writing: ECT_REG_DCSTART0\n");
    }

    tc = htoel(CyclTime);

    rc = ecx_FPWR(context->port, slaveh, ECT_REG_DCCYCLE0, sizeof(tc), &tc, EC_TIMEOUTRET); /* SYNC0 cycle time */
    if (rc <= 0) {
        printf("Error writing: ECT_REG_DCCYCLE0\n");
    }
    rc = ecx_FPWR(context->port, slaveh, ECT_REG_DCSYNCACT, sizeof(RA), &RA,
 ...
 */
void
ecx_dcsync01(ecx_contextt *context, uint16 slave, boolean act, uint32 CyclTime0, uint32 CyclTime1, int32 CyclShift) {
    uint8 h, RA;
    uint16 slaveh;
    int64 t, t1;
    int32 tc;
    ...
    t1 = 0;
    (void) ecx_FPRD(context->port, slaveh, ECT_REG_DCSYSTIME, sizeof(t1), &t1,
                    EC_TIMEOUTRET); /* read local time of slave */
    t1 = etohll(t1);
    ...
    (void) ecx_FPWR(context->port, slaveh, ECT_REG_DCSYNCACT, sizeof(RA), &RA,
                    EC_TIMEOUTRET); /* activate cyclic operation */
...
//    946684800UL - this is an offset in SECONDS;

    mastertime.sec -= 946684800UL;  /* EtherCAT uses 2000-01-01 as epoch start instead of 1970-01-01 */

    //this should be 1000000000 - no you idiot it shouldn't
    mastertime64 = (((uint64) mastertime.sec * 1000000) + (uint64) mastertime.usec) * 1000;


    UM_INFO(GBEM_UM_EN,
            "GBEM: EtherCAT master time (nsecs from 2000-01-01 as epoch start instead of 1970-01-01) :%12lld",
            mastertime64);


    char buf[80];
    ctime_r((time_t *) &mastertime.sec, buf);
    buf[strlen(buf) - 1] = '\0';
    UM_INFO(GBEM_UM_EN, "GBEM: EtherCAT master time is [%s]", buf);

//    struct tm  ts;
//    char       buf[80];
//    // Format time, "ddd yyyy-mm-dd hh:mm:ss zzz"
//    ts = *localtime(&mastertime.sec);
//    strftime(buf, sizeof(buf), "%a %Y-%m-%d %H:%M:%S %Z", &ts);
//    UM_INFO(GBEM_UM_EN, "GBEM: EtherCAT master time: %s", buf);
