...
void ecx_dcsync0(ecx_contextt *context, uint16 slave, boolean act, uint32 CyclTime, int32 CyclShift)
{
   ...
   t1 = 0;
 rc = ecx_FPRD(context->port, slaveh, ECT_REG_DCSYSTIME, sizeof(t1), &t1, EC_TIMEOUTRET); /* read local time of slave */
    if (rc <= 0) {
   ...
   t = htoell(t);
    ...
    rc = ecx_FPWR(context->port, slaveh, ECT_REG_DCSYNCACT, sizeof(RA), &RA, EC_TIMEOUTRET); /* activate cyclic operation */
    if (rc <= 0) {
        printf("Error writing: ECT_REG_DCSYNCACT\n");
    }
...
//    946684800UL - this is an offset in SECONDS;
   ...
   mastertime64 = (((uint64)mastertime.sec * 1000000) + (uint64)mastertime.usec) * 1000;
    ...
    UM_INFO(GBEM_UM_EN, "GBEM: EtherCAT master time (nsecs from 2000-01-01 as epoch start instead of 1970-01-01) :%12lld", mastertime64);
