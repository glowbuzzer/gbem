...
void os_get_array_of_optional_slaves(machine_config_optional_slaves_t config, bool *optional_slave) {
    ...
    for (uint8_t i = 0; i < MAX_NUM_OPTIONAL_SLAVES; i++) {
        optional_slave[i] = config.enable_optional_slave[i];
    }
    ...
    for (uint8_t i = 0; i < MAP_NUM_SLAVES; i++) {
        if (which_slaves_are_optional[i] == true) {
            ...
            if (enable_slaves_from_config[count_of_config] == false) {
                ...
                os_delete_and_shift(map_slave_standard_sdo_function_ptr, sizeof(gberror_t (*)(uint16_t)),
                                    map_num_slaves,
                                    i - count_of_deleted);
                // MAP_SLAVE_CUSTOM_FMMU_SM_FUNCTIONS
                os_delete_and_shift(map_slave_custom_fmmu_sm_function_ptr, sizeof(gberror_t (*)(uint16_t)),
                                    map_num_slaves,
                                    i - count_of_deleted);
                // MAP_SLAVE_DC_TYPE
                ...
                os_delete_and_shift(map_slave_fsoe_slave_type, sizeof(enum FSOE_SLAVE_TYPE), map_num_slaves,
                                    i - count_of_deleted);
                ...
                os_delete_and_shift(map_slave_fsoe_get_master_state_function_ptr,
                                    sizeof(gberror_t (*)(uint16_t slave,
...
//    map_fsoe_master_slot_to_slave
    ...
    UM_INFO(GBEM_UM_EN, "GBEM: Number of slaves remaining enabled [%u] (originally [%u])", map_num_slaves,
            MAP_NUM_SLAVES);
