...
gberror_t ec_standard_sdos_asda_a2(const uint16_t slave) {
    ...
    if (!ec_sdo_write_int32(slave, ASDA_A2_INTERPOLATION_TIME_UNITS_SDO_INDEX, ASDA_A2_INTERPOLATION_TIME_UNITS_SDO_SUB_INDEX, MAP_CYCLE_TIME)) {
        return E_SDO_WRITE_FAILURE;
    }
    ...
    if (!ec_sdo_write_uint32(slave, ASDA_A2_POSITION_FACTOR_NUMERATOR_SDO_INDEX,
                            ASDA_A2_POSITION_FACTOR_NUMERATOR_SDO_SUB_INDEX, ASDA_A2_POSITION_FACTOR_NUMERATOR_VALUE)) {
        return E_SDO_WRITE_FAILURE;
    }
...
uint8_t *ec_get_error_string_sdo_asda_a2(const uint16_t drive) {
    ...
    if (!ec_sdo_read_uint16(map_drive_to_slave[drive], ASDA_A2_ERROR_CODE_SDO_INDEX, ASDA_A2_ERROR_CODE_SDO_SUB_INDEX, &drive_error_code)) {

        if (drive_error_code==0){
            sprintf(error_code_string, "ASDA_A2 no error on drive");
        ...
        for (int i =0; i<NUM_ASDA_A2_ERROR_CODES; i++){
            if (asda_a2_error_code_description[i].numeric_code_1==drive_error_code){
                sprintf(error_code_string, "%s", asda_a2_error_code_description );
                return error_code_string;
            }
        }

        sprintf(error_code_string, "ASDA_A2 error code returned by drive did not match any in the error table");
        return error_code_string;
    }
    sprintf(error_code_string, "ASDA_A2 error code returned by drive did not match any in the error table");
