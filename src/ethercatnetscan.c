...
bool netscan_pdo_map_failure = false;
        ...
        for (idxloop = 1; idxloop <= nidx; idxloop++) {
            ...
            if (idx > 0) {
                ...
                for (subidxloop = 1; subidxloop <= subidx; subidxloop++) {
                    ...
                    printf("  [%-11u] [0x%4.4X.%1d] 0x%4.4X:0x%2.2X 0x%2.2X", byteoffset, abs_offset, abs_bit, obj_idx,
                           obj_subidx,
                           bitlen);
...
int si_map_sii(int slave) {
    ...
    if (retVal == 0) {
        printf("No PDO mapping found in SII\n");
    }
    return retVal;
}
...
void ecm_netscan(char *ifname1, bool apply_pdo_mapping) {
    ...
    if (ec_init(ifname1)) {
        ...
        if (ec_config_init(FALSE)) {
            ...
            if (apply_pdo_mapping) {
                ...
                if (ec_statecheck(0, EC_STATE_PRE_OP, 8 * EC_TIMEOUTSTATE) == EC_STATE_PRE_OP) {
                    UM_INFO(GBEM_UM_EN, "GBEM: All slaves transitioned to PRE OP state)");
            ...
            for (cnt = 1; cnt <= ec_slavecount; cnt++) {
                ...
                printf(" Activeports: %d.%d.%d.%d\n", (ec_slave[cnt].activeports & 0x01) > 0,
                       (ec_slave[cnt].activeports & 0x02) > 0, (ec_slave[cnt].activeports & 0x04) > 0,
                ...
                for (j = 0; j < ec_slave[cnt].FMMUunused; j++) {
                    printf(" FMMU%1d Ls:%8.8x Ll:%4d Lsb:%d Leb:%d Ps:%4.4x Psb:%d Ty:%2.2x Act:%2.2x\n", j,
                           ...
                           etohs(ec_slave[cnt].FMMU[j].PhysStart), ec_slave[cnt].FMMU[j].PhysStartBit,
        ...
        ecm_status.
                net_scan_state = ECM_NET_SCAN_ERROR;
    }
}
